...
### With a11y-bridge (sighted mode)

```
$ a11y-bridge oc projects
...
### With a11y-bridge (screen reader mode)
...
This is where a11y-bridge provides the most value. Kubernetes/OpenShift table output is the single hardest format for screen readers.
...
```
$ a11y-bridge oc get pods -n rhtas-tenant
...
If any pods had status issues (and the output had color), a11y-bridge would add semantic prefixes:
...
```
$ a11y-bridge --domain screen-reader oc describe pod my-pod -n rhtas-tenant
...
```
$ a11y-bridge --domain screen-reader oc get pods -n nonexistent-ns
...
```
$ a11y-bridge oc get pods -n nonexistent-ns
status: Running: oc get pods -n nonexistent-ns
...
```
$ a11y-bridge --domain screen-reader oc status -n rhtas-tenant
...
a11y-bridge is pipe-safe — when stdout is not a terminal, it passes raw output through without labels:

```bash
# Count pods by status
a11y-bridge oc get pods -n rhtas-tenant | awk '{print $3}' | sort | uniq -c

# Find failing pods
a11y-bridge oc get pods -n rhtas-tenant | grep -v Completed

# Use in scripts
POD_COUNT=$(a11y-bridge oc get pods -n rhtas-tenant | tail -n +2 | wc -l)
...
## Summary: What oc Lacks vs What a11y-bridge Adds

| oc behavior | CLI-ACS gap | a11y-bridge compensation |
|---|---|---|
...
| Respects NO_COLOR but no --no-color flag | CV-3: Missing per-invocation flag | a11y-bridge always strips ANSI regardless |
| No structured output for many commands (oc projects, oc status) | OS-6: No --json for some commands | a11y-bridge formats output with labels |
...
a11y-bridge --domain screen-reader oc get pods -n myns

# Force screen reader mode via env var
NO_COLOR=1 a11y-bridge oc get pods -n myns
A11Y_SCREEN_READER=1 a11y-bridge oc get pods -n myns

# Audio mode — speaks results aloud
a11y-bridge --domain audio oc get pods -n myns

# AI session with voice — describe what you want, hear results
a11y-bridge --domain audio --askai oc
...
a11y-bridge --raw oc describe pod my-pod -n myns

# Pass through with NO_COLOR=1 TERM=dumb set
a11y-bridge --passthrough oc diff -f manifest.yaml
```
